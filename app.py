...
def model_predict(img_path, model):
    ...
    new_arr = cv2.resize(img,(28,28))
    new_arr = np.array(new_arr/255)
    new_arr = new_arr.reshape(-1, 28, 28, 3)
...
def index():
    ...
    file_path = "my-app/src/uploads/crop.png"
    file.save(file_path)

    if file is None or file.filename == "":
        return {"error": "no file"}
        
    preds = model_predict("my-app/src/uploads/crop.png", model)
    pred_class = preds.argmax()              
    data = labels[pred_class]

    content = "Just give me the names of the most 3 commonly used fertilizers for " + data['name'] + " crop disease in India without any acknowlegment and explanation"
